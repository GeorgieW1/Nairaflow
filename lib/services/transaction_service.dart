...
class TransactionService {
  ...
  // Subscribe to TV
  static Future<Transaction> subscribeTV({
    ...
    try {
      ...
      if (e.toString().contains('DioException')) {
        throw Exception('TV subscription failed: Unable to connect to server');
      }
      throw Exception('TV subscription failed: ${e.toString()}');
    }
  }

  // Purchase E-pin
  static Future<Transaction> purchaseEpin({
    ...
    try {
      ...
      if (response.data['success'] == true) {
        ...
        final transaction = Transaction.fromJson({
          ...transactionData,
          'userId': await _getCurrentUserId(),
          'type': TransactionType.epin.name,
          'description': '$category E-pin Purchase',
          'updatedAt': DateTime.now().toIso8601String(),
        });
      ...
      if (e.toString().contains('DioException')) {
        throw Exception('E-pin purchase failed: Unable to connect to server');
      }
      throw Exception('E-pin purchase failed: ${e.toString()}');
    }
  }
